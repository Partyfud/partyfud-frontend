...
export default function EventTypes() {
  ...
  const handleEventTypeClick = (eventTypeName: string) => {
    ...
    if (matchingOccasion) {
      ...
      router.push(`/user/packages?occasion_name=${encodeURIComponent(matchingOccasion.name)}`);
    } else {
      // Fallback: navigate with the event type name as occasion_name
      router.push(`/user/packages?occasion_name=${encodeURIComponent(eventTypeName)}`);
    }
  };
